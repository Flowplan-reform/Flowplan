  ...
  auth: {
    signInWithPassword: async({email,password})=>{
      try{
        ...
        if(password.length>=6) return {data:{user:{email}}};
        throw new Error("Passwort muss mind. 6 Zeichen haben");
      }
    },
...
function ProgramsScreen({exercises,user,showToast}){
  const [customs,setCustoms]=useState(()=>{try{return [];}catch{return[];}});
  const [building,setBuilding]=useState(false);
  const [editing,setEditing]=useState(null);
  const [timer,setTimer]=useState(null);
  const [log,setLog]=useState(()=>{try{return [];}catch{return[];}});

  const saveCustoms=(c)=>{setCustoms(c);};
  const saveLog=(l)=>{setLog(l);};
...
export default function App(){
  ...
  const [favs,setFavs]=useState(()=>{try{return [];}catch{return[];}});
  const [toast,setToast]=useState("");

  const showToast=(msg)=>{setToast(msg);setTimeout(()=>setToast(""),2600);};
  const toggleFav=(id)=>{const n=favs.includes(id)?favs.filter(f=>f!==id):[...favs,id];setFavs(n);};
